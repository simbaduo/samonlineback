...
namespace test.Controllers
{
  ...
  public class FindCarController : ControllerBase
  {
    ...
    [HttpGet("/model/{modelTerm}")]
    public async Task<ActionResult> modelSearch(string modelTerm)
    {
      // Call asynchronous network methods in a try/catch block to handle exceptions.
      try
      {
        client.DefaultRequestHeaders.Host = "marketcheck-prod.apigee.net";
        HttpResponseMessage response = await client.GetAsync($"http://api.marketcheck.com/v2/search/car/auto-complete?api_key=4Nqu6uJ7mimviznvAjdE9uHmqsCEBfnz&field=model&input={modelTerm}");
        response.EnsureSuccessStatusCode();
